...
export default function Onboard() {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="flex-1 pb-24 pt-[50px]">
        <ImageSlide
      ...
      {currentIndex === onboardImages.length - 1 ? (
        // Último slide - botão único centralizado
        <View className=" items-center justify-center px-4 pb-[40px]">
          <TouchableOpacity
            className="w-full flex-row items-center justify-center gap-2 rounded-lg bg-cor-primaria px-[24px] py-[10px]"
            accessibilityRole="button"
            onPress={handleStart}>
            <Text className="font-mulish-medium text-[16px] font-medium text-white md:text-lg">
              Começar Agora
            </Text>
          </TouchableOpacity>
        </View>
      ) : (
        // Slides anteriores - dois botões
        <View
          className={`absolute bottom-10 left-0 right-0 flex-row px-4 md:bottom-12 md:px-5 ${isFirstSlide ? 'justify-end' : 'justify-between'}`}>
          {!isFirstSlide && (
            <TouchableOpacity
          ...
          <TouchableOpacity
            className="w-[140px] flex-row items-center justify-center gap-3 rounded-lg bg-cor-primaria px-6 py-[10px]"
            accessibilityRole="button"
            onPress={handleNext}>
            <Text className="font-mulish-medium text-[16px] font-medium text-white md:text-lg">
              Próximo
